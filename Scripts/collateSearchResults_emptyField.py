...
for searchBand in sorted_searchBands:
    ...
    if ( searchBand == sorted_searchBands[-1] or jobNum < sorted_searchBands[ segmentCounter + 1 ][2] ):
        ...
        if loudestLine:
            ...
            indent(loudestEl, 1)
            SubElement( searchJobElement,"num_templates_vetoed").text = str( IFOvetoed )
            indent(loudestEl, 1)
...
search_bands_xml.write("search_bands_py.xml", xml_declaration=True, encoding='UTF-8', method='html')
...
# TODO  Merge IFO-vetoed bands with previous veto bands (not strictly necessary)
if len(ifo_v_jobs):   # Append ifo vetoed jobs to vetoed bands file (if they exist)
    ...
    veto_bands_xml.write("veto_bands_py.xml", xml_declaration=True, encoding='UTF-8', method='html')
