...
run_exec 'lalapps_ConvertToSFTv2', {
    ...
    'fmin'      => $ul_freq - 0.5*2*$ul_band,
    'fmax'      => $ul_freq + 1.5*2*$ul_band,
    'outputDir' => $banded_sft_dir,
};

# inverse metric diagonal elements
my @diag_inv_metric = (
		       300   / (PI**2 * $span_time**2), # 10*df
		       6480  / (PI**2 * $span_time**4), # 6*df1dot
		       25200 / (PI**2 * $span_time**6), # 6*df2dot
		       #72900 / (PI**2 * $span_time**8), # initial stab at 6*df3dot
		       );
...
for (my $i = 0; $i < $injections; ++$i) {
    ...
    #{
	#my $f3_min = (2*$max_braking - 1) * $max_braking * ($freq[1]**3 / $freq[0]**2);
	#my $f3_max = (2*$min_braking - 1) * $min_braking * ($freq[1]**3 / $freq[0]**2);
	#$freq[3] = $f3_min + rand() * ($f3_max - $f3_min);
    #}
	...
	run_exec 'lalapps_Makefakedata_v4', {
	    ...
	    'fmin'            => sprintf("%.2f", $srch_freq[0] - 1*$srch_pad),
    ...
    my @loudest = run_exec_output 'lalapps_ComputeFstatistic_v2', {
	'refTime'             => $start_time,
	'Alpha'               => $right_ascension,
	'Delta'               => $declination,
	'Freq'                => $srch_freq[0],
