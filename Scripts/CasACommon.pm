...
#use Devel::PatchPerl;
#
#Devel::PatchPerl->patch_source( '5.10.1', '/home/ra/scripts/svn/cw/docs/O1/CoherentDirected/Scripts/perl-5.10.1' );
...
sub parse_sft_filename {
    ...
    $name =~ m|^([A-Z])-(\d+)_(\1\d)_(\w+)_(\w*)-*(\d{10})-(\d+)\.sft$|  or # fscan SFTs have different filename conventions... 
...
sub parse_CFSv2_output_line {
    ...
    croak "'$line' is not 'freq alpha delta f1dot f2dot f3dot twoF log10BSGL twoFH1 twoFL1'" if @tokens != 10;
